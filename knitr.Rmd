...
```{r missingvalues}
Totalmissing <- sum(is.na(activity$date)) +sum(is.na(activity$interval)) + sum(is.na(activity$steps))
NoNa <- activity
NoNa$steps = transform(activity, steps = ifelse(is.na(steps), mean(steps, na.rm=TRUE), steps))

stepst2<- aggregate(steps ~ date,  data= NoNa, sum)

meanstepsPerDay2 <- format(mean(NoNa$steps), scientific = FALSE)

medianstepsPerDay2 <- median(NoNa$steps)

phist2 <- ggplot(data=stepst, aes(NoNa$steps)) + 
  geom_histogram(
                 col="red", 
                 fill="green", 
                 alpha = .2) + 
